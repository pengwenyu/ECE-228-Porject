...
train_id = open("train/ids.txt").read().splitlines()
test_id = open("test/ids.txt").read().splitlines()
#train_id = [int (i) for i in train_id ]
#test_id = [int (i) for i in test_id ]
print('LEN of training,testing',len(train_id),len(test_id))
with open(anno, "r") as j:
    annotations = json.load(j)
types = annotations["types"]
imgs = annotations["imgs"]
imgs = {id:imgs[id] for id in train_id}
...
for id, (img, info) in tqdm(enumerate(imgs.items(), 1)):
